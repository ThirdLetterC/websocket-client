...
static uint16_t parse_port_or_default(const char *port_text, uint16_t default_port) {
...
int main(int argc, char **argv) {
    ...
    auto host = (argc > first_arg_index) ? argv[first_arg_index] : "echo.websocket.org";
    auto port = parse_port_or_default((argc > first_arg_index + 1) ? argv[first_arg_index + 1] : nullptr, 443);
    auto path = (argc > first_arg_index + 2) ? argv[first_arg_index + 2] : "/.ws";
    auto message = (argc > first_arg_index + 3) ? argv[first_arg_index + 3] : "hello from c23 websocket client";
    ...
    if (!binary_mode) {
        ...
        size_t received = 0;
        if (!ws_client_receive_text(client, receive_buffer, sizeof(receive_buffer), &received)) {
        ...
        constexpr uint8_t binary_payload[] = {0x00U, 0x11U, 0x22U, 0x33U, 0x44U, 0x55U};
        if (!ws_client_send_binary(client, binary_payload, sizeof(binary_payload))) {
        ...
        size_t received = 0;
        if (!ws_client_receive_binary(client, receive_buffer, sizeof(receive_buffer), &received)) {
