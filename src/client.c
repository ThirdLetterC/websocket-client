...
static void ws_sha1_transform(sha1_ctx_t *ctx, const uint8_t *block) {
    ...
    for (size_t i = 0; i < 16; ++i) {
        auto offset = i * 4;
        w[i] = ((uint32_t)block[offset] << 24U) | ((uint32_t)block[offset + 1] << 16U) |
...
static void ws_sha1_update(sha1_ctx_t *ctx, const uint8_t *data, size_t length) {
    if (length == 0) {
        return;
    }
...
[[nodiscard]] static size_t ws_base64_encode(const uint8_t *input, size_t input_length,
    ...
    while (src + 2U < input_length) {
        auto chunk = ((uint32_t)input[src] << 16U) | ((uint32_t)input[src + 1] << 8U) |
                     (uint32_t)input[src + 2];
...
[[nodiscard]] static bool ws_read_exact(int fd, WOLFSSL *ssl, bool use_tls, uint8_t *buffer,
    ...
    size_t total = 0;
    while (total < length) {
        auto received = ws_transport_recv(fd, ssl, use_tls, buffer + total, length - total);
        if (received <= 0) {
            return false;
        }
...
[[nodiscard]] static bool ws_send_all(int fd, WOLFSSL *ssl, bool use_tls, const uint8_t *buffer,
    ...
    size_t total = 0;
    while (total < length) {
        auto sent = ws_transport_send(fd, ssl, use_tls, buffer + total, length - total);
        if (sent <= 0) {
            return false;
        }
        total += (size_t)sent;
...
[[nodiscard]] static bool ws_discard_bytes(int fd, WOLFSSL *ssl, bool use_tls, uint64_t length) {
    ...
    while (remaining > 0) {
        auto chunk = (remaining > sizeof(scratch)) ? sizeof(scratch) : (size_t)remaining;
...
[[nodiscard]] static bool ws_connect_tcp(const char *host, uint16_t port, int *out_fd) {
    char port_text[6] = {0};
...
[[nodiscard]] static bool ws_find_header_value(const char *response, const char *header_name,
...
[[nodiscard]] static bool ws_header_has_token(const char *value, const char *token) {
...
[[nodiscard]] static bool ws_compute_accept_value(const char *key, char *output,
    ...
    auto printed = snprintf(challenge, sizeof(challenge), "%s%s", key, WS_ACCEPT_GUID);
...
[[nodiscard]] static bool ws_send_frame(ws_client_t *client, uint8_t opcode,
    ...
    if (payload_length > SIZE_MAX - (2 + length_field_size + 4)) {
        ws_set_error(client, "Frame payload is too large");
        return false;
    }

    auto frame_length = 2 + length_field_size + 4 + payload_length;
    ...
    auto ok = ws_send_all(client->socket_fd, client->ssl, client->use_tls, frame, frame_length);
    free(frame);
...
[[nodiscard]] static bool ws_client_connect_impl(ws_client_t *client, const char *host,
    ...
    auto request_length = snprintf(
        ...
        "\r\n",
        path, host, (unsigned)port, key_encoded, (unsigned)WS_HANDSHAKE_VERSION);
    ...
    if (!ws_find_header_value(response, "Upgrade", upgrade_header, sizeof(upgrade_header)) ||
...
[[nodiscard]] bool ws_client_connect(ws_client_t *client, const char *host, uint16_t port,
                                     const char *path) {
    return ws_client_connect_impl(client, host, port, path, false);
}

[[nodiscard]] bool ws_client_connect_secure(ws_client_t *client, const char *host, uint16_t port,
                                            const char *path) {
...
[[nodiscard]] static bool ws_client_receive_data(ws_client_t *client, uint8_t expected_opcode,
    ...
    while (true) {
        ...
        if (payload_length == 126U) {
            ...
            if (!ws_read_exact(client->socket_fd, client->ssl, client->use_tls, extended,
                               sizeof(extended))) {
        ...
        if (opcode == 0x9U) {
            ...
            if (!ws_read_exact(client->socket_fd, client->ssl, client->use_tls, ping_payload,
...
[[nodiscard]] bool ws_client_receive_text(ws_client_t *client, char *buffer, size_t capacity,
                                          size_t *out_length) {
    if (client == nullptr || buffer == nullptr || capacity == 0) {
        return false;
    }

    return ws_client_receive_data(client, 0x1U, "text", (uint8_t *)buffer, capacity, out_length,
                                  true);
}

[[nodiscard]] bool ws_client_receive_binary(ws_client_t *client, uint8_t *buffer, size_t capacity,
                                            size_t *out_length) {
    if (client == nullptr || buffer == nullptr || capacity == 0) {
        return false;
    }

    return ws_client_receive_data(client, 0x2U, "binary", buffer, capacity, out_length, false);
}
